...
TCHAR *MONITOR_FILE_TYPE = _T(".rvt");
...
{
	MessageBoxA(NULL, "a", "InjectInfo", MB_OK);
	//return;
	...
	//------------hook api----------------
	hMod = GetModuleHandle(L"Roamer.exe");
	MessageBoxA(NULL, "a1", "InjectInfo", MB_OK);
	pDosHeader = (PIMAGE_DOS_HEADER)hMod;
	pNTHeaders = (PIMAGE_NT_HEADERS)((BYTE *)hMod + pDosHeader->e_lfanew);
	pOptHeader = (PIMAGE_OPTIONAL_HEADER)&(pNTHeaders->OptionalHeader);
	MessageBoxA(NULL, "a2", "InjectInfo", MB_OK);
	pImportDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)((BYTE *)hMod + pOptHeader->DataDirectory[1].VirtualAddress);
	MessageBoxA(NULL, "a3", "InjectInfo", MB_OK);
	//return;
	while(pImportDescriptor->FirstThunk)
	{MessageBoxA(NULL, "a4", "InjectInfo", MB_OK);
		char * dllname = (char *)((BYTE *)hMod + pImportDescriptor->Name);
		MessageBoxA(NULL, "a5", "InjectInfo", MB_OK);
		pThunkData = (PIMAGE_THUNK_DATA)((BYTE *)hMod + pImportDescriptor->OriginalFirstThunk);
		MessageBoxA(NULL, "a6", "InjectInfo", MB_OK);
		int no = 1;
		while(pThunkData->u1.Function)
		{MessageBoxA(NULL, "a7", "InjectInfo", MB_OK);
			...
			MessageBoxA(NULL, "a8", "InjectInfo", MB_OK);
			//修改内存的部分
